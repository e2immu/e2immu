...
public class StatementAnalyser implements HasNavigationData<StatementAnalyser> {
    ...
    private AnalysisStatus apply(SharedState sharedState,
        ...
        Set<Variable> valueOrLink = SetUtil.immutableUnion(evaluationResult.valueChanges().keySet(),
                evaluationResult.linkedVariables().keySet());

        // first assignments, because they may create a new level in VariableInfoContainer
        for (Variable variable : valueOrLink) {
            EvaluationResult.ExpressionChangeData valueChangeData = evaluationResult.valueChanges().get(variable);
            Set<Variable> linkTo = evaluationResult.linkedVariables().get(variable);
            ...
            vic.assignment(level, VariableInfoImpl.NOT_A_VARIABLE_FIELD); // FIXME
    ...
    private AnalysisStatus step1_initialisation(SharedState sharedState) {
        ...
        if (structure.localVariableCreation != null) {
            ...
            vic.assignment(l1, VariableInfoImpl.NOT_A_VARIABLE_FIELD);
        ...
        for (Expression initialiser : structure.initialisers) {
            if (initialiser instanceof LocalVariableCreation lvc) {
                ...
                if (assignedInLoop) {
                    ...
                    saToCreate.findOrCreateL1(analyserContext, lvr, sharedState.evaluationContext.getInitialStatementTime()); // "touch" it
                }
            }
            try {
        ...
        try {
            ...
            if (status == DELAYS) {
                if (statementAnalysis.statement instanceof ReturnStatement) {
                    ...
                    variableInfo.assignment(VariableInfoContainer.LEVEL_3_EVALUATION, VariableInfoImpl.NOT_A_VARIABLE_FIELD);
    ...
    private void step3_ReturnConditionally(SharedState sharedState, Expression value) {
        ...
        vic.assignment(VariableInfoContainer.LEVEL_3_EVALUATION, VariableInfoImpl.NOT_A_VARIABLE_FIELD);
    ...
    public record ModificationData(StatementAnalyserResult.Builder builder, int level,
                                   Set<Variable> wroteValueOrLinkedVariables) {
        public int level(Variable variable) {
            if (wroteValueOrLinkedVariables.contains(variable)) return level;
            return VariableInfoContainer.LEVEL_1_INITIALISER;
        }
    }
    ...
    public class SetProperty implements StatementAnalysisModification {
        ...
        public void accept(ModificationData modificationData) {
            statementAnalysis.addProperty(analyserContext, modificationData.level(variable), variable, property, value);
    ...
    public class MarkRead implements StatementAnalysisModification {
        ...
        @Override
        public void accept(ModificationData modificationData) {
            statementAnalysis.findForWriting(analyserContext, variable).markRead(modificationData.level(variable));
        }
