    ...
    private void computeRemapParameters(boolean modifyingMainMethod) {
        ...
        for (ParameterInfo parameterInfo : companionMethod.methodInspection.get().getParameters()) {
            ...
            if (aspectVariables >= 1 && parameterInfo.index == 0) {
                ...
                value = new MethodCall(scope, aspectMethod, List.of(), ObjectFlow.NO_FLOW);
                ...
                Expression scope = new VariableExpression(new This(analyserContext, aspectMethod.typeInfo));
                MethodCall methodValue = new MethodCall(scope, aspectMethod, List.of(), ObjectFlow.NO_FLOW);
    ...
    private class EvaluationContextImpl extends AbstractEvaluationContextImpl {
        ...
        public CurrentValueResult currentValue(Variable variable, int statementTime, boolean isNotAssignmentTarget) {
            if (variable instanceof ParameterInfo parameterInfo) {
                ...
                if (remapping == null) return new CurrentValueResult(EmptyExpression.NO_VALUE, null); // delay!
                return new CurrentValueResult(Objects.requireNonNull(remapping.get(parameterInfo.name)), null);
            }
            return new CurrentValueResult(new VariableExpression(variable), null);
        }
