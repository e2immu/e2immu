...
public class StatementAnalysis extends AbstractAnalysisBuilder implements Comparable<StatementAnalysis>,
                ...
                if (merged.add(fqn)) {
                    ...
                    if (toMerge.size() > 0) {
                        try {
                            ...
                            (TODO find out how 2 and 3 interplay, they may be mostly equivalent)
                             */
     ...
     */
    private void addConditionalAssignmentCopy(VariableInfo variableInfo) {
        String suffix = VariableInfoContainerImpl.conditionalInitializationSuffix(variableInfo);
        String fqn = variableInfo.variable().fullyQualifiedName() + suffix;
    ...
    public VariableInfoContainer createVariable(EvaluationContext evaluationContext,
        ...
        if (variable instanceof ReturnVariable returnVariable) {
            ...
            vic.setInitialValue(initialValue.expression, initialValue.expressionIsDelayed, propertyMap, true);
            valueProperties.forEach((k, v) -> vic.setProperty(k, v, false, INITIAL));
        }
        return vic;
    }
