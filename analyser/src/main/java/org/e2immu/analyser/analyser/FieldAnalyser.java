...
public class FieldAnalyser extends AbstractAnalyser {
        ...
        if (isFinal == Level.TRUE && MultiLevel.value(finalNotNullValue, MultiLevel.NOT_NULL) == MultiLevel.EFFECTIVE) {
            ...
            if (methodsWhereFieldIsAssigned.size() > 0 && !haveInitialiser) {
                ...
                boolean linkingAndPreconditionsComputed = methodsWhereFieldIsAssigned.stream()
                        .allMatch(m -> m.methodAnalysis.variablesLinkedToFieldsAndParameters.isSet() && m.methodAnalysis.precondition.isSet());
                ...
                boolean linkedToVarsInPrecondition = methodsWhereFieldIsAssigned.stream().allMatch(m ->
                        m.methodAnalysis.precondition.isSet() &&
                                !Collections.disjoint(safeLinkedVariables(m.methodAnalysis.fieldSummaries.get(fieldInfo)),
                ...
                if (linkedToVarsInPrecondition) {
                    ...
                    boolean allCompatible = methodsWhereFieldIsAssigned.stream().allMatch(m -> {
                        Value assignment = m.methodAnalysis.fieldSummaries.get(fieldInfo).value.get();
    ...
    private List<MethodAnalyser> methodsWhereFieldIsAssigned() {
        return allMethodsAndConstructors.stream()
                .filter(m -> m.methodAnalysis.fieldSummaries.isSet(fieldInfo))
                .filter(m -> m.methodAnalysis.fieldSummaries.get(fieldInfo).getProperty(VariableProperty.ASSIGNED) >= Level.TRUE)
        ...
        if (fieldInspection.modifiers.contains(FieldModifier.PRIVATE)) {
            if (!fieldInfo.isStatic()) {
                ...
                int readInMethods = allMethodsAndConstructors.stream()
                        ...
                        .filter(m -> m.methodAnalysis.fieldSummaries.isSet(fieldInfo)) // field seen
                        .mapToInt(m -> m.methodAnalysis.fieldSummaries.get(fieldInfo).properties.getOtherwise(VariableProperty.READ, Level.FALSE))
    ...
    private boolean someAssignmentValuesUndefined(VariableProperty property) {
        boolean allAssignmentValuesDefined = allMethodsAndConstructors.stream().allMatch(m ->
                ...
                !m.methodAnalysis.fieldSummaries.isSet(fieldInfo) ||
                        // field is not assigned to in the method
                        m.methodAnalysis.fieldSummaries.get(fieldInfo).getProperty(VariableProperty.ASSIGNED) < Level.TRUE ||
                        // if it is present, assigned to, it needs to have a value
                        m.methodAnalysis.fieldSummaries.get(fieldInfo).value.isSet());
    ...
    private boolean delaysOnFieldSummariesResolved() {
        return allMethodsAndConstructors.stream()
                .filter(m -> m.methodAnalysis.fieldSummaries.isSet(fieldInfo))
                .map(m -> m.methodAnalysis.fieldSummaries.get(fieldInfo))
    ...
    private int computeValueFromContext(VariableProperty property, boolean allDelaysResolved) {
        IntStream contextRestrictions = allMethodsAndConstructors.stream()
                .filter(m -> m.methodAnalysis.fieldSummaries.isSet(fieldInfo))
                .mapToInt(m -> m.methodAnalysis.fieldSummaries.get(fieldInfo).getProperty(property));
    ...
    private int computeValueFromAssignment(boolean haveInitialiser, Value value,
        ...
        allMethodsAndConstructors.forEach(m -> {
            if (m.methodAnalysis.fieldSummaries.isSet(fieldInfo)) {
                TransferValue tv = m.methodAnalysis.fieldSummaries.get(fieldInfo);
                if (tv.value.isSet()) {
    ...
    private boolean analyseFinalValue(Value value,
        ...
        if (!(fieldInfo.isExplicitlyFinal() && haveInitialiser)) {
            ...
            for (MethodAnalyser methodAnalyser : myMethodsAndConstructors) {
                if (methodAnalyser.methodAnalysis.fieldSummaries.isSet(fieldInfo)) {
                    TransferValue tv = methodAnalyser.methodAnalysis.fieldSummaries.get(fieldInfo);
    ...
    private boolean analyseLinked() {
        ...
        boolean allDefined = allMethodsAndConstructors.stream()
                .allMatch(m ->
                        m.methodAnalysis.variablesLinkedToFieldsAndParameters.isSet() && (
                                !m.methodAnalysis.fieldSummaries.isSet(fieldInfo) ||
                                        m.methodAnalysis.fieldSummaries.get(fieldInfo).linkedVariables.isSet()));
        if (!allDefined) {
            if (Logger.isLogEnabled(DELAYED)) {
                log(DELAYED, "VariablesLinkedToFieldsAndParameters not yet set for methods: [{}]",
                        allMethodsAndConstructors.stream()
                                .filter(m -> !m.methodAnalysis.variablesLinkedToFieldsAndParameters.isSet())
        ...
        allMethodsAndConstructors.stream()
                .filter(m -> m.methodAnalysis.fieldSummaries.isSet(fieldInfo))
                .filter(m -> m.methodAnalysis.fieldSummaries.get(fieldInfo).linkedVariables.isSet())
                .forEach(m -> links.addAll(m.methodAnalysis.fieldSummaries.get(fieldInfo).linkedVariables.get()));
    ...
    private boolean analyseFinal(boolean fieldCanBeWrittenFromOutsideThisType,
        ...
        int isAssignedOutsideConstructors = allMethodsAndConstructors.stream()
                ...
                .filter(m -> m.methodAnalysis.fieldSummaries.isSet(fieldInfo))
                .mapToInt(m -> m.methodAnalysis.fieldSummaries.get(fieldInfo).getProperty(VariableProperty.ASSIGNED))
    ...
    private boolean analyseModified(boolean fieldCanBeWrittenFromOutsideThisType, boolean fieldSummariesNotYetSet) {
        ...
        boolean allContentModificationsDefined = allMethodsAndConstructors.stream().allMatch(m ->
                !m.methodAnalysis.fieldSummaries.isSet(fieldInfo) ||
                        m.methodAnalysis.fieldSummaries.get(fieldInfo).getProperty(VariableProperty.READ) < Level.TRUE ||
                        m.methodAnalysis.fieldSummaries.get(fieldInfo).getProperty(VariableProperty.MODIFIED) != Level.DELAY);

        if (allContentModificationsDefined) {
            boolean modified = fieldCanBeWrittenFromOutsideThisType ||
                    allMethodsAndConstructors.stream()
                            .filter(m -> m.methodAnalysis.fieldSummaries.isSet(fieldInfo))
                            .filter(m -> m.methodAnalysis.fieldSummaries.get(fieldInfo).getProperty(VariableProperty.READ) >= Level.TRUE)
                            .anyMatch(m -> m.methodAnalysis.fieldSummaries.get(fieldInfo).getProperty(VariableProperty.MODIFIED) == Level.TRUE);
