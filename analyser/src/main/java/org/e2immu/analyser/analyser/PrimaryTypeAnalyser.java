...
public class TypeAnalyser implements Analyser {
    ...
    public final List<Pair<FieldInfo, FieldAnalyser>> fieldAnalysers;
    public final List<Pair<MethodInfo, MethodAnalyser>> methodAnalysers;
    public final List<Pair<TypeInfo, TypeAnalyser>> subTypeAnalysers;


    public final Map<WithInspectionAndAnalysis, Analyser> analyserMap;
    public final List<Pair<WithInspectionAndAnalysis, Analyser>> analysers;

    public TypeAnalyser(SortedType sortedType, Configuration configuration, @NotNull E2ImmuAnnotationExpressions e2ImmuAnnotationExpressions) {
        this.configuration = Objects.requireNonNull(configuration);
        this.e2ImmuAnnotationExpressions = Objects.requireNonNull(e2ImmuAnnotationExpressions);

        this.typeInfo = Objects.requireNonNull(sortedType.primaryType);
        assert this.typeInfo.isPrimaryType();

        typeInspection = typeInfo.typeInspection.get();
        typeAnalysis = new TypeAnalysis(typeInfo.hasBeenDefined(), typeInfo.simpleName, typeInspection.annotationMode, new Location(typeInfo), typeInfo.isInterface());

        fieldAnalysers = typeInspection.fields.stream().map(fieldInfo -> new Pair<>(fieldInfo, new FieldAnalyser())).collect(Collectors.toUnmodifiableList());
        methodAnalysers = typeInspection.methods.stream().map(methodInfo -> new Pair<>(methodInfo, new MethodAnalyser())).collect(Collectors.toUnmodifiableList());

        ImmutableMap.Builder<WithInspectionAndAnalysis, Analyser> builder = new ImmutableMap.Builder<>();
        fieldAnalysers.forEach(pair -> builder.put(pair.k, pair.v));
        methodAnalysers.forEach(pair -> builder.put(pair.k, pair.v));

        analyserMap = builder.build();

        analysers = sortedType.methodsFieldsSubTypes.stream().map(mfs -> new Pair<>(mfs, analyserMap.get(mfs))).collect(Collectors.toList());
