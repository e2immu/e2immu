    ...
    public void test20() throws IOException {
            ...
            if ("example1".equals(d.methodInfo().name)) {
                ...
                if (d.variable() instanceof FieldReference fr && "s2".equals(fr.fieldInfo.name)) {
                    ...
                    if ("2".equals(d.statementId())) {
                        String expected = d.iteration() <= 7 ? "<f:s2>" : "instance type HashSet<String>";
                        assertEquals(expected, d.currentValue().toString());

                        String linked = switch (d.iteration()) {
                            case 0, 1, 2, 3, 4, 5, 6 -> "c.set:-1,c:-1,localD.set:-1,localD:-1,return example1:-1";
                            default -> "";
                        };
                ...
                if (d.variable() instanceof FieldReference fr && "set".equals(fr.fieldInfo.name)) {
                    ...
                    if ("2".equals(d.statementId())) {
                        String expectValue = d.iteration() <= 7 ? "<f:set>" : "nullable instance type Set<String>";
                        assertEquals(expectValue, d.currentValue().toString());
                    }
                }
                if ("c".equals(d.variableName())) {
                    if ("0".equals(d.statementId())) {
                        String expectValue = d.iteration() <= 7 ? "<new:C1>" : "new C1(s2)";
                        mustSeeIteration(d, 6);
                        assertEquals(expectValue, d.currentValue().toString());
                        String expectLinked = switch (d.iteration()) {
                            case 0, 1, 2, 3, 4, 5, 6 -> "this.s2:-1";
                            default -> "";
                        };
        ...
        TypeAnalyserVisitor typeAnalyserVisitor = d -> {
            if ("C1".equals(d.typeInfo().simpleName)) {
                ...
                assertDv(d, 6, MultiLevel.EFFECTIVELY_E2IMMUTABLE_DV, Property.IMMUTABLE);
            }
        ...
        StatementAnalyserVisitor statementAnalyserVisitor = d -> {
            if ("0".equals(d.statementId())) {
                assertEquals(Set.of(4, 6).contains(d.iteration()), d.context().evaluationContext().allowBreakDelay());
            }
        };
