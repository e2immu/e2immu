                ...
                if (d.variable() instanceof ReturnVariable) {
                    if ("9".equals(d.statementId())) {
                        String expected = switch (d.iteration()) {
                            ...
                            case 1 -> "(<c:boolean>?<m:apply>:<m:apply>&&<m:length>>=1)&&!<c:boolean>&&<vp:NEWLINE:container@Record_Space>!=<m:get>&&-1+<vl:end>>=(-1+end$8>=pos$8&&list.get(pos$8)!=<vp:NEWLINE:container@Record_Space>?1+(<c:boolean>?<vl:pos>:<c:boolean>?<new:ForwardInfo>:<m:length>>=1?<vl:pos>:instance type int):start)";
                            case 2 -> "(<c:boolean>?<m:apply>:<m:apply>&&<m:length>>=1)&&!<c:boolean>&&<vp:NEWLINE:cm@Parameter_split;mom@Parameter_split>!=<m:get>&&-1+<vl:end>>=(-1+end$8>=pos$8&&list.get(pos$8)!=<vp:NEWLINE:cm@Parameter_split;mom@Parameter_split>?1+(<c:boolean>?instance type int:<c:boolean>?<new:ForwardInfo>:<m:length>>=1?<vl:pos>:instance type int):start)";
