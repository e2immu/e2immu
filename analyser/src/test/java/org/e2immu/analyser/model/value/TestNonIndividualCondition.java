...
public class TestNonIndividualCondition extends CommonAbstractValue {
    ...
    public void test1() {
        ...
        Assert.assertEquals("null==s||null==p", rest(orValue2, Filter.FilterMode.ACCEPT).minimalOutput());

        Expression andValue = newAndAppend(sEqualsNull, pEqualsNull);
        Assert.assertEquals("null==s&&null==p", rest(andValue, Filter.FilterMode.REJECT).toString());
