...
public class TestLoopStatementChecks extends CommonTestRunner {
    ...
    MethodAnalyserVisitor methodAnalyserVisitor = d -> {
        if ("method3".equals(d.methodInfo().name)) {
            ...
            Assert.assertEquals(MultiLevel.EFFECTIVELY_NOT_NULL, tv.getProperty(VariableProperty.NOT_NULL)); // (2)
        ...
        if ("method3bis".equals(d.methodInfo().name)) {
            VariableInfo tv = d.getReturnAsVariable();
            Assert.assertNotNull(tv);
            Assert.assertEquals(MultiLevel.NULLABLE, tv.getProperty(VariableProperty.NOT_NULL));
        }
    };
