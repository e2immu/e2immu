...
When encountering a return statement with a value different from NO_VALUE, the statement analyser freezes the value and its properties in a value or `VariableValue`, and stores it in the statement:
...
| SIZE | delay | not a size | >= 0 (is a size) | = 0 (empty) | >= 1 (not empty) | = 1 | >= 2
...
It stores the value objects representing the condition, the `ifTrue` and the `ifFalse` side, and delays evaluation as much as possible.
...
Each time the statement analyser registers a field, it takes the values from this map and places them in the property-value map `OldAboutVariable`.
...
`IMMUTABLE`, `CONTAINER`, `NOT_NULL`, `SIZE`, `SIZE_COPY`, `IDENTITY`, `FLUENT`.
When the transfer value's `value` object is constant, will the method analyser copy it into the single return value of the method.
When the method is static and there is only one return statement, and it is possible to in-line it, this will be done.
...
We want to be able to inline this method.
The fact that the method is an identity makes this very easy.
Somehow we need to attach the `@NotNull` to this inlined value.

We will use a `PropertyWrapper` which wraps the value but pre-empts the property, in this particular case, `NOT_NULL`.

==== Size restrictions

The value system will try to reduce size restrictions as 'greater or equal to _x_', or 'equal to _x_'.
This means that:

[options=header]
|===
| original | is transformed into
...
We have three types of wrappers.

. `NegatedValue`, which works on non-constant numeric or boolean values, always `NOT_NULL`.
. `ConstrainedNumericValue`, which can wrap non-constant numeric values, always `NOT_NULL`.
. `PropertyWrapper`, which can add the `NOT_NULL` or `SIZE` property.

The `NegatedValue` wrapper can wrap the `ConstrainedNumericValue` wrapper; it must always be on the outside, so that

[source]
----
set.size(),?>=0 + not (set.size(),?>=0)
----

can be resolved to 0.

A `PropertyWrapper` wrapping `ConstrainedNumericValue` would only be relevant with `SIZE` but this can resolve itself using a new `ConstrainedNumericValue`:
if `set.size(),?>=0` is wrapped with a `@Size(min = 3)` property on `set`, we can return `set.size(),?>=3`.
Note that this only works with a minimum restriction, not an equals one, because otherwise `set.size()` would have been reduced to the value of that equals restriction.

A `PropertyWrapper` wrapping a `NegatedValue` for `SIZE` without a `ConstrainedNumericValue` is not possible.

Summarizing, the only wrapper combination possible is `NegatedValue(ConstrainedNumericValue(value))`.
