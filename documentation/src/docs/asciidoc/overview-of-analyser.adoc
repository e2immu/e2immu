...
The property `METHOD_DELAY` is a special one, in the sense that it keeps track of properties of methods called themselves being delayed.
Method properties that rely on `METHOD_DELAY` before they are fixed: `NOT_MODIFIED`, SIZE, NOT_NULL. #TODO is this correct for SIZE, NN?#
...
| IMMUTABLE | delay | not E1 | E1Immutable | E1, not E2 | E2Immutable | |
| SIZE | delay | >= 0 (no info) | = 0 (empty) | >= 1 (not empty) | = 1 | >= 2 | = 2
| NOT_NULL | delay | false | @NotNull | NN, not NN1^*^ | @NotNull1 | NN1, not NN2^*^ | @NotNull2
|===
