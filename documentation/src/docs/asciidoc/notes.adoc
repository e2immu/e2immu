...
. every modifying method must have at least one precondition x~i~.
It will be marked `@Only(before="xi")`
...
. mark tentative preconditions that are based on fields, when we know the `@Final` situation of the fields, and there is at least one `@Variable` (non-`@Final`) field
. if the type analyser has _approved_ these preconditions, then we annotate with `@Only()`or `@Mark`.
For `@Mark` the method must assign to one of the fields in the precondition.
