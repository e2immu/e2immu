...
public class ParameterInspection {
    ...
    public static class ParameterInspectionBuilder {
        ...
        public ParameterInspection build() {
            return new ParameterInspection(List.copyOf(parameterNames), varArgs);
        }
    }
}
----
<1> because the constructor is private, and only called in the `build` method with an `List.copyOf` method, we can be sure that `parameterNames` is effectively immutable
...
public void build(@NullNotAllowed2 @Independent Handler<AsyncResult<Set<T>>> next) {
    ...
    ...
    next.handle(Future.succeededFuture(Set.copyOf(set)));
}
----
