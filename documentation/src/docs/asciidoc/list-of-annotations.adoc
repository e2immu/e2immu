...
When added to a constructor, the annotation implies that the resulting instance is not linked to the parameters of the constructor.
If a method does not return a real value (it is `void`, <<identity-annotation>> or <<fluent-annotation>>), and it has consumer parameters, the annotation indicates that the objects linked to the consumers are not linked to the fields or parameters of the class.
The annotation is then added to the consumer, typically together with <<nullnotallowed2-annotation>>.

Linking means that if a given object is modified, the linked object is modified too.
