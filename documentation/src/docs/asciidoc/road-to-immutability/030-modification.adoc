...
* `Consumer` because it is a functional interface (an interface with a single abstract method); we will add an {exposed} annotation rather than {nm}.

The definitions of modification given in this section are sufficient to follow the narrative, but they will be refined in sections <<modification-part2>>
