...
* `Consumer` because it is a functional interface (an interface with a single abstract method or SAM); they are {nm} by definition.

In order to keep the narrative going, we defer a discussion of modification in the context of parameters of functional interface types to the section <<indirect-modification>>.
