...
The linking to `b` will now depend on the properties of the single abstract method:
...
Most of the other rules follow from the basic rules above.
For example,
...
We have detailed in <<functional-types>> why functional types are compatible with the container concept.
Now we must verify if they are compatible with the first rule of level 2 immutability: all fields must be {nm}.

#TODO TODO are we certain that functional types of `T` are support data?# Are they support data types?
In the case of `Lazy<T>`, the field of type `Supplier<T>` obviously is.
So the functional interface should also obey rules 2 and 3.

As for rule 2: being either private, or {e2immutable} itself.
Can functional types be {e2immutable}?
We simply need to follow the rules, looking at the functional type as a sub-type (static, or inner class).

As for rule 3, independence.
A consumer parameter can serve to smuggle out objects!
Consider two visitor-methods added to the safe set-based container of version 3 above:
...
class SetBasedContainer3<T> {
    ...
    public void safeVisit(Consumer<T> consumer) {
        data.forEach(consumer); // <2>
    }
}
----
<1> The consumer exposes `data` to the outside world.
<2> The consumer exposes `T` elements to the outside world.
...
The computations that the analyser needs to track dynamic type annotations, are similar to those it needs to compute eventual immutability, as introduced in the next chapter.
