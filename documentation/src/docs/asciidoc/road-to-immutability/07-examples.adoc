...
public class Lazy<T> {
    ...
    public T get() {
        ...
        synchronized (this) {
            if (t == null) {
                t = Objects.requireNonNull(supplier.get()); // <2>
            }
            return t;
        }
    }
...
----
<1> The `@Mark("xx")` annotation as detected by the analyser currently implies `@Only(before="xx")`.
...
<2> Here `t` links to `supplier`, as explained in <<linking-formally>>.
...
Because it is of an unbound generic type, the field `t` then becomes `@NotModified`.
