...
Even {nm} is not needed, because after the mark, the `get` method is not accessible anymore.
...
Calling `get` may cause a modification in `supplier`.
...
The `Supplier` parameter and field can be modifiable or not; after the mark, the `get` method is not accessible anymore.
