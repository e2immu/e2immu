...
* `Consumer` because it is a functional type, with only one method, which is always `@NotModified`, as detailed further on.

The rules for not having to write `@NotModified` will be detailed later, but this example pretty much covers all the bases.

=== The case of functional types
...
However, this particular `accept` method is modifying its input, causing the `sets` parameter to be modified.
