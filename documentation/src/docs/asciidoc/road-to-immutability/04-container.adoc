...
The main rule for containers stipulates that the parameter has to be {nm}, which is the case.
So it seems easy to conclude that functional parameters can safely be passed as an argument to containers.

The main argument for going along with this is predictability and control: the container will not modify your objects.
If the implementation of the consumer modifies your objects, well, you were the one providing the container with the means of modification.
