...
****
*Additional, overriding modification rule*: We treat unbound parameter types as level 2 immutable when passing them as parameters to abstract methods.
...
Once there is modifying method, the analyser cannot exclude that the unknown code will not execute it.
Therefore,

****
*Implication of the indirect modification rule*: A method with parameters of abstract or interface type is modifying as soon as there are other non-private modifying methods in the type.
****
