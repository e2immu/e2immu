...
* when entering a conditional block, positive side (`if`), start a new `ConditionManager` with the statement's expression added to the current condition and state;
